...
func TestCreateWorkspace_RejectsDuplicateName(t *testing.T) {
	...
	cmd := app.createWorkspace("existing-ws", repos, bases, "", "", false, true)
	msg := cmd()
...
func TestCreateWorkspace_RejectsBranchExists(t *testing.T) {
	...
	cmd := app.createWorkspace("taken-branch", repos, bases, "", "", false, true)
	msg := cmd()
...
func TestCreateWorkspace_PassesValidationForUniqueName(t *testing.T) {
	...
	cmd := app.createWorkspace("unique-ws", repos, bases, "", "", false, true)
	msg := cmd()
...
func TestCreateWorkspace_PersistsGroup(t *testing.T) {
	...
	repos := []data.RepoRef{{Path: repo, Name: "repo"}}
	bases := []string{"main"}
	cmd := app.createWorkspace("test-group-ws", repos, bases, "", "shipping-q2", false, true)
	msg := cmd()
